...
function extractKeyWords(item) {
  ...
  const highPriorityWords = [
    ...
    // Loại sản phẩm và thiết bị ngoại vi
    "laptop",
    "desktop",
    "server",
    "workstation",
  ...
  const words = cleanedItem
    ...
    .filter(
      (word) =>
        word.length > 1 && !commonWords.includes(word) && /[a-z0-9]/.test(word) // Giữ từ khóa có số hoặc chữ
    )
    .map((word) => ({
      text: word.trim(),
